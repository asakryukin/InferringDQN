...
#     """Returns a session that will use <num_cpu> CPUs only"""
#     tf_config = tf.ConfigProto(
#         inter_op_parallelism_threads=num_cpu,
#         intra_op_parallelism_threads=num_cpu)
#     return tf.Session(config=tf_config)

# Force 0 GPU (alternative config above - the original baselines version)
def make_session(num_cpu):
    """Returns a session that will use <num_cpu> CPUs only"""
    # tf_config = tf.ConfigProto(
    #     device_count = {'GPU': 0})
...
def noisy_dense(x, size, name, bias=False, activation_fn=tf.identity):
