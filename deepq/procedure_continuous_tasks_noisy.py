...
def learn_continuous_tasks_noisy(env,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for joint_i in range(num_action_streams):
            names.append([])
            names[-1].append("deepq/q_func/action_value/" + str(joint_i) + "128/w_sigma:0")
            names[-1].append("deepq/q_func/action_value/" + str(joint_i) + "out/w_sigma:0")
        mus = []
        for nl in names:
            mus.append([])
            for n in nl:
        ...
        pickle.dump(weights_mean, open(str(log_index) + "q_stat_wmu99_" + log_prefix + ".pkl", 'wb'),
                    protocol=pickle.HIGHEST_PROTOCOL)
        pickle.dump(weights_stds, open(str(log_index) + "q_stat_wsig99_" + log_prefix + ".pkl", 'wb'),
                    protocol=pickle.HIGHEST_PROTOCOL)
